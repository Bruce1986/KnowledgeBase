 ...
 * 【元規則：框架進化】 (Meta-Rule: Framework Evolution)：
   ...
   * 觸發： 當你在執行任務時，若發現本框架（_AI_MANIFEST.md 中的任何規則）存在「模糊地帶」、「效率低下」或「阻礙了更高品質的分析」時...
   * 行動： 你被鼓勵在完成當前任務後，於你的回覆中，明確提出一個「框架修正建議」。
   * 建議格式： 你的建議必須包含以下三個部分：
     * 【框架修正建議】
     * 【觀察到的問題】：(例如：我發現「規則 4」的日誌格式在回溯時不易閱讀)
     * 【建議的修正】：(例如：我建議將日誌標題 ### [YYYY-MM-DD HH:mm] 改為 #### ... 以降低層級)
