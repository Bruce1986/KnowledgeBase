...
- **`/10_PROBLEMS/`**：存放所有「問題（Problem）」的筆記。
    - 檔名格式：`P-XXX-主題.md`，其中 `XXX` 為三位數的流水號（例如 `001`、`002`、...）。
        - **主題命名**：主題應使用連字號 `-` 取代空格，並避免使用 `\ / : * ? " < > |` 等檔案系統保留字元。
...
- 儲存位置 (Storage)：
  你的所有分析和回應，必須被追加 (Append) 到當前互動筆記的 ## 99. AI 分析日誌 (AI Log) 區塊下。必須使用以下格式包含時間戳（此為 Obsidian 模板語法，請直接輸出此字串）：
...
- 【元規則：框架進化】 (Meta-Rule: Framework Evolution)：
  ...
  - **建議格式**：你的建議必須包含以下兩個部分：
    ...
    2. **【建議的修改】**：(例如：在 `GEMINI.md` 的「狀態標籤定義」中，為「問題筆記」新增一個「擱置」狀態，用於標示已確認但暫時無法處理的問題。)
